use std::{iter::Scan, collections::HashMap, vec};
...
impl Scanner {
    ...
    pub fn scan_token(&mut self){
        ...
        match c {
            ...
            '!' => {
                if self.matcher('=') {self.add_token(TokenType::NotEqual)} else {self.add_token(TokenType::Not)};
            },
            '=' => {
                if self.matcher('=') {self.add_token(TokenType::Equality)} else {self.add_token(TokenType::Assignment)}
            }
            '<' => {
                if self.matcher('=') {self.add_token(TokenType::LessEqual)} else {self.add_token(TokenType::Less)}
            }
            '>' => {
                if self.matcher('=') {self.add_token(TokenType::GreaterEqual)} else {self.add_token(TokenType::Greater)}
            }
            '#' => {//Comments are #
                while(self.peek() != '\n' && !self.is_at_end()){
                    self.advance();
    ...
    fn add_literal_token(&mut self, token_type : TokenType, literal : Option<Literal>){
        ...
        self.tokens.push(Token{ token_type, literal, lex: lex.to_string(), line: self.line })
    }
    ...
    fn number(&mut self) {
        ...
        self.add_literal_token(TokenType::Number, Some(Literal::Number(self.source[self.start..self.current].parse::<f64>().unwrap())))
    }
